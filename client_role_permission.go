...
func (c *defaultClient) AssignPermissionToRole(role string) error {
	switch role {
	case "Admin":
		permissions, err := c.allPermissionGet()
		if err != nil {
			log.Print("error cannot get all permissions")
			return err
		}
		...
		if tx.Error != nil {
			log.Print("error query ", tx.Error)
			return tx.Error
		}
