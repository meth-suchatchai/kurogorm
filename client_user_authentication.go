...
func (c *defaultClient) GetUserAuthenticationByMobile(mobileNumber, countryCode string) (*dbmodels.UserAuthentication, error) {
	...
	log.Printf("getUserAuthentication data: %v, %v", mobileNumber, countryCode)
	exec := c.orm.Where("mobile_number = ? AND country_code = ?", mobileNumber, countryCode).First(&userAuth)
	if exec.Error != nil {
		log.Printf("get user failed: %v", exec.Error)
...
func (c *defaultClient) GetUserAuthenticationById(userId uint, token string) (*dbmodels.UserAuthentication, error) {
	var userAuth = &dbmodels.UserAuthentication{}
	log.Printf("getUserAuthentication data: %v token: %v", userId, token)
...
func (c *defaultClient) CreateUserAuthentication(data *dbmodels.UserAuthentication) (*dbmodels.UserAuthentication, error) {
	...
	if result.Error != nil {
		log.Printf("create user authentication: %v", result.Error)
...
func (c *defaultClient) UpdateUserAuthentication(field *dbmodels.UpdateUserAuthentication) bool {
	...
	if exec.Error != nil {
		log.Printf("update user authentication failed: %v", exec.Error)
		return false
	}
...
func (c *defaultClient) CreateOrUpdateUserAuthentication(user *dbmodels.User, auth *dbmodels.UpdateUserAuthentication) error {
	...
	if exec.Error != nil {
		if exec.Error == gorm.ErrRecordNotFound {
			...
			tx.Create(&m)
			if tx.Error != nil {
				log.Printf("CreateOrUpdateUserAuthentication create user authentication failed: %v", tx.Error)
				tx.Rollback()
				return tx.Error
			}
			return nil
		} else {
			log.Printf("CreateOrUpdateUserAuthentication error: %v", tx.Error)
			tx.Rollback()
			return tx.Error
		}
	} else {
		...
		if tx.Error != nil {
			log.Printf("CreateOrUpdateUserAuthentication update user authentication failed: %v", tx.Error)
			tx.Rollback()
			return tx.Error
		}
